...
namespace VersopayBackend.Dtos
{
    public class UsuarioUpdateDto : IValidatableObject
    {
        ...
        [Required] public string Documento { get; set; } = default!;
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext _)
        {
            ...
            if (TipoCadastro == TipoCadastro.PF && digits.Length != 11)
                yield return new("CPF deve ter 11 dígitos.", new[] { nameof(Documento) });
            if (TipoCadastro == TipoCadastro.PJ && digits.Length != 14)
                yield return new("CNPJ deve ter 14 dígitos.", new[] { nameof(Documento) });
        }
    }
}
