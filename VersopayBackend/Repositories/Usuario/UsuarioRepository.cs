...
namespace VersopayBackend.Repositories
{
    public sealed class UsuarioRepository(AppDbContext db) : IUsuarioRepository
    {
        ...
        public Task<bool> CpfCnpjExistsAsync(string cpfCnpjDigits, CancellationToken ct) =>
            db.Usuarios.AnyAsync(u => u.CpfCnpj == cpfCnpjDigits, ct);

        public Task<Usuario?> FindByIdAsync(int id, CancellationToken ct) =>
            db.Usuarios.FirstOrDefaultAsync(u => u.Id == id, ct);

        public Task AddAsync(Usuario u, CancellationToken ct) =>
            db.Usuarios.AddAsync(u, ct).AsTask();
    }
}
