...
namespace VersopayBackend.Repositories
{
    public sealed class ProviderCredentialRepository(AppDbContext db) : IProviderCredentialRepository
    {
        ...
        public async Task<string?> GetWebhookSecretAsync(int ownerUserId, PaymentProvider provider, CancellationToken ct)
        {
            var cred = await db.Set<ProviderCredential>()
                .FirstOrDefaultAsync(x => x.OwnerUserId == ownerUserId && x.Provider == provider, ct);
            return cred?.WebhookSignatureSecret;
        }

        public async Task<(string? current, string? previous)> GetWebhookSecretsPairAsync(int ownerUserId, PaymentProvider provider, CancellationToken ct)
        {
            var cred = await db.Set<ProviderCredential>()
                .FirstOrDefaultAsync(x => x.OwnerUserId == ownerUserId && x.Provider == provider, ct);
