...
");
            ...
            // =========================
            // USUARIOS (2/4): backfill dinâmico a partir de CpfCnpj (se existir) + soltar deps + drop CpfCnpj
            // =========================
            migrationBuilder.Sql(@"
IF COL_LENGTH(N'[dbo].[Usuarios]', N'CpfCnpj') IS NOT NULL
BEGIN
    -- backfill DINÂMICO (evita bind na compilação)
    ...
    -- dropar índices que toquem CpfCnpj
    ...
    -- dropar DEFAULT em CpfCnpj (se houver)
    ...
    -- dropar CHECKs que mencionem CpfCnpj
...
");
            ...
            // =========================
            // USUARIOS (4/4): check constraint PF/PJ (100% dinâmico/isolado) — CORRIGIDO
            // =========================
            migrationBuilder.Sql(@"
...
BEGIN
    ...
    -- soltar deps e dropar coluna antiga
...
");
            ...
            // =========================
            // FATURAMENTOS: add + backfill + drop legada + índices
            // =========================
            migrationBuilder.Sql(@"
...
BEGIN
    ...
    -- soltar deps em CpfCnpj e dropar
    DECLARE @dropIdx NVARCHAR(MAX), @df sysname, @sqlDrop NVARCHAR(MAX);
...
");
            ...
            // =========================
            // TRANSFERENCIAS: add colunas + remover default + índices
            // =========================
            migrationBuilder.Sql(@"
...
");
            ...
            // =========================
            // INBOUND WEBHOOK LOG: criar tabela (se não existir) + índice único
            // =========================
            migrationBuilder.Sql(@"
